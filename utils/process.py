...
def load_data(dataset_str='wiki'): # {'pubmed', 'citeseer', 'cora'}
    """
    ...
    int.dataset_str.train.index => the indices of training instances in graph, for the inductive setting as list object.
    ...
    for i in range(len(names)):
        with open("wiki_data/ind.{}.{}".format(dataset_str, names[i]), 'rb') as f:
            if sys.version_info > (3, 0):
                objects.append(pkl.load(f, encoding='latin1'))
            else:
                objects.append(pkl.load(f))
        test_idx_reorder = parse_index_file("wiki_data/ind.{}.test.index".format(dataset_str))
        train_idx_reorder = parse_index_file("wiki_data/ind.{}.train.index".format(dataset_str))
